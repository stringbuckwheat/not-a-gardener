...
public interface PlantWateringService {
    /* 물주기 기록 추가 */
    AfterWatering add(WateringRequest wateringRequest, Pageable pageable);

    /* insert, update 이후 리턴할 DTO를 만드는 메소드 */
    AfterWatering getAfterWatering(Long plantId, Pageable pageable);

    /* 한 식물의 물주기 기록 */
    List<WateringForOnePlant> getAll(Long plantId, Pageable pageable);

    /* 물주기 수정 */
    AfterWatering modify(WateringRequest wateringRequest, Pageable pageable);
