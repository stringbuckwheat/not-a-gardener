...
import { cilRestaurant, cilFlower, cilMugTea, cilDrop, cilSmile, cilFaceDead, cilHandPointUp, cilSwimming, cilZoom } from "@coreui/icons";
import { Link } from "react-router-dom";
import CIcon from '@coreui/icons-react';
import { useNavigate } from "react-router-dom";
import GardenTag from "src/pages/garden/GardenTag";
const GardenCard = (props) => {
    console.log("garden card props", props.plant);
    ...
    return (
        <CCol md={4} xs={12}>
            <CWidgetStatsF
                className="mb-3"
                color={color}
                icon={<CIcon icon={icon} height={40} title={"hi"}/>}
                title={
                    <>
                        <div><small>{plant.plantSpecies}</small></div>
                        <GardenTag plant={plant}/>
                        <div><small>{wateringMsg}</small></div>
                    </>}
                value={
